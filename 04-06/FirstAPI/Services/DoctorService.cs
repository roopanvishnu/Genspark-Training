...
public class DoctorService : IDoctorService
{
    ...
    public async Task<Doctor> AddDoctor(DoctorAddRequestDTO doctor)
    {
        ...
        try
        {
            ...
            Doctor newDoc = _doctorMapper.MapDoctorAddRequestDoctor(doctor);
            ...
            if (doctor.Specialities != null && doctor.Specialities.Count > 0)
            {
                ...
                {
                    DoctorSpeciality ds = _specialityMapper.MapDoctorSpeciality(doc.Id, specIds[i]);
    ...
    public async Task<int[]> MapAndAddSpeciality(DoctorAddRequestDTO doctor)
    {
        ...
        int count = 0;
            ...
            foreach (var speciality in doctor.Specialities!)
            {
                ...
                if (spec == null)
                {
                    spec = await _specialityRepository.Add(_specialityMapper.MapSpecialityAddRequestDoctor(speciality));
                }
    ...
    {
        var docList = await _doctorRepository.GetAll();
        Doctor? doc = docList.FirstOrDefault(d => d.Email == email);
        if (doc == null)
        {
            throw new Exception("No doctor found");
        }
