...
builder.Services.AddScoped<IAppointmentService, InMemoryAppointmentService>();
...
// Consolidate to just one registration
builder.Services.AddAuthorization(options =>
{
    options.AddPolicy("Minimum3YearsExperience", policy =>
...
builder.Services.AddAuthorization(options =>
{
    options.AddPolicy("ExperiencedDoctorOnly", policy =>
        ...
        {
            // Example: only DoctorId 1 and 2 are "experienced"
            var doctorIdClaim = context.User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            return doctorIdClaim != null && (doctorIdClaim == "1" || doctorIdClaim == "2");
        }));
});
