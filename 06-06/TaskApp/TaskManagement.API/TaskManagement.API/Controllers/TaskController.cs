...
public class TaskController : ControllerBase
{
    ...
    {
        var deletedTasks = await _taskService.GetDeletedTasksAsync();
        ...
        return Ok(new
        {
            ...
            resultsCount = deletedTasks.Count,
            data = deletedTasks
        });
    }
...
public async Task<IActionResult> UploadAttachment(Guid id, IFormFile file)
{
    ...
    {
        _logger.LogWarning("No file provided for task ID: {TaskId}", id);
    ...
    {
        _logger.LogWarning("File too large for task ID: {TaskId}, Size: {FileSize}", id, file.Length);
    ...
    {
        _logger.LogError(ex, "Error uploading attachment for task ID: {TaskId}", id);
    ...
    public async Task<IActionResult> UpdateStatus(Guid id, [FromBody] TeamTaskUpdateDto dto)
    {
        var userId = User.FindFirstValue(ClaimTypes.NameIdentifier)!;

        _logger.LogInformation("TeamMember updating task status - Task ID: {TaskId}, User ID: {UserId}, New Status: {Status}", id, userId, dto.Status);

        var success = await _taskService.UpdateStatusByTeamMemberAsync(id, userId, dto.Status, dto.Comment);
        if (!success)
        {
            _logger.LogWarning("Failed to update task status - Task ID: {TaskId}, User ID: {UserId}", id, userId);
            return BadRequest(new { success = false, message = "Failed to update status. You might not be assigned to this task." });
        }
