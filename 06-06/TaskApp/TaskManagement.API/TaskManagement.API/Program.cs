...
// });
// */
// var corsPolicy = "_frontendCorsPolicy";
// builder.Services.AddCors(options =>
// {
...
var jwtSecret = config["JwtSettings:Secret"];
var connectionString = config.GetConnectionString("DefaultConnection");
var keyVaultName = "roopanvishnu5754";
var keyVaultUri = new Uri($"https://{keyVaultName}.vault.azure.net/");
builder.Configuration.AddAzureKeyVault(keyVaultUri, new DefaultAzureCredential());

// 🔥 2. Print debug logs (test output before Serilog even starts)
Console.WriteLine("✅ Program started");
Console.WriteLine("🔍 Trying to read 'AzureStorage' from Key Vault...");
var azureStorageFromVault = builder.Configuration["AzureStorage"];
if (string.IsNullOrWhiteSpace(azureStorageFromVault))
{
    Console.WriteLine("❌ AzureStorage not found in Key Vault!");
}
else
{
    Console.WriteLine("✅ AzureStorage secret retrieved successfully from Key Vault");
}
...
builder.Host.UseSerilog((context, loggerConfig) =>
{
    ...
    var azureStorage = config["AzureStorage"];
    ...
    loggerConfig
        ...
        .WriteTo.Console()
        .WriteTo.File("Logs/local-log-.txt", rollingInterval: RollingInterval.Day);
    ...
    if (!string.IsNullOrWhiteSpace(azureStorage))
    {
        loggerConfig.WriteTo.AzureBlobStorage(
            ...
            storageFileName: "application/{yyyy}/{MM}/{dd}/log-{HH}.log",
